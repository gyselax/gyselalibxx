...
TEST(AdvectionFieldRThetaComputation, TestAdvectionFieldFinder)
{
    ...
    ddc::for_each(grid, [&](IdxRTheta const irtheta) {
        EXPECT_LE(std::abs(ddcHelper::get<X>(difference_between_fields_exact_and_xy)(irtheta)), 1e-5);
        EXPECT_LE(std::abs(ddcHelper::get<Y>(difference_between_fields_exact_and_xy)(irtheta)), 1e-5);

        EXPECT_LE(std::abs(ddcHelper::get<X>(difference_between_fields_xy_and_rtheta)(irtheta)), 1e-13);
        EXPECT_LE(std::abs(ddcHelper::get<Y>(difference_between_fields_xy_and_rtheta)(irtheta)), 1e-13);
    });
