...
TEST(PartialDerivative, SplinePartialDerivative1DDx)
{
    ...
    Spline1DPartialDerivativeCreator<SplineXBuilder, SplineXEvaluator>
            preallocator_partial_dx(builder_x, spline_evaluator_x);

    DFieldMemXY field_to_differentiate(idxrange_xy);
    DFieldXY field_differentiated(preallocator_partial_dx(get_const_field(field_to_differentiate)));

    ddc::parallel_fill(field_differentiated, 1.);
}
