...
TEST(DerivFieldTest, derivFieldValueAccess)
{
    ...
    // Collect the index range of the sliced fields
    IdxRange<dX, GridX, GridY> slice_dx_idx_range = get_idx_range(dxdyField_const_copy[slice_idx_dx]);
    IdxRange<dY, GridX, GridY> slice_dy_idx_range = get_idx_range(dxdyField_const_copy[slice_idx_dy]);
...
TEST(DerivFieldMemTest, FieldDeepCopy)
{
    ...
    DField<IdxRange<GridX, GridY>, Kokkos::HostSpace, Kokkos::layout_stride> values
            = dxdyField.get_values_field();
    DField<IdxRange<GridY>, Kokkos::HostSpace, Kokkos::layout_stride> left_x_derivs
            = dxdyField[IdxXdX(first_deriv, deriv_idx_range_x.front())];
    DField<IdxRange<GridY>, Kokkos::HostSpace, Kokkos::layout_stride> right_x_derivs
