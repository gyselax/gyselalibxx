...
[**ClassList**](annotated.md) **>** [**DiscretePoloidalCSSplineMapping**](classDiscretePoloidalCSSplineMapping.md)
...
|  KOKKOS\_FUNCTION Coord&lt; [**X**](structX.md), [**Y**](structY.md) &gt; | [**operator()**](#function-operator) (Coord&lt; [**curvilinear\_tag\_r**](classDiscretePoloidalCSSplineMapping.md#typedef-curvilinear_tag_r), [**curvilinear\_tag\_theta**](classDiscretePoloidalCSSplineMapping.md#typedef-curvilinear_tag_theta) &gt; const & coord) const<br>_Compute the physical coordinates from the logical coordinates._  |
...
The mapping describe here is only defined on a grid. The [**DiscretePoloidalCSSplineMapping**](classDiscretePoloidalCSSplineMapping.md) class decomposes the mapping on B-splines to evaluate it on the physical domain.
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::CoordArg =  Coord<R, Theta>;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::CoordJacobian =  CoordArg;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::R_cov =  typename R::Dual;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::Theta_cov =  typename Theta::Dual;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::X_cov =  typename X::Dual;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::Y_cov =  typename Y::Dual;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::cartesian_tag_x =  X;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::cartesian_tag_y =  Y;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::curvilinear_tag_r =  R;
```
...
```C++
using DiscretePoloidalCSSplineMapping< X, Y, SplineEvaluator, R, Theta, MemorySpace >::curvilinear_tag_theta =  Theta;
```
...
### function DiscretePoloidalCSSplineMapping 

_Instantiate a_ [_**DiscretePoloidalCSSplineMapping**_](classDiscretePoloidalCSSplineMapping.md) _from the coefficients of 2D splines approximating the mapping._
```C++
inline DiscretePoloidalCSSplineMapping::DiscretePoloidalCSSplineMapping (
...
A discrete mapping is a mapping whose values are known only at the mesh points of the grid. To interpolate the mapping, we use B-splines. The [**DiscretePoloidalCSSplineMapping**](classDiscretePoloidalCSSplineMapping.md) mapping is initialised from the coefficients in front of the basis splines which arise when we approximate the functions \(x(r,\theta)\), and \(y(r,\theta)\) (with \(x\) and \(y\) the physical dimensions in the logical domain) with Splines (using SplineBuilder2D). Then to interpolate the mapping, we will evaluate the decomposed functions on B-splines (see DiscretePoloidalCSSplineMapping::operator()).
...
**See also:** DiscretePoloidalCSSplineMapping::operator() 
...
```C++
inline KOKKOS_INLINE_FUNCTION const Coord< X , Y > DiscretePoloidalCSSplineMapping::control_point (
...
inline void DiscretePoloidalCSSplineMapping::control_points (
...
```C++
inline KOKKOS_INLINE_FUNCTION DTensor < VectorIndexSet< X , Y >, VectorIndexSet< R_cov , Theta_cov > > DiscretePoloidalCSSplineMapping::first_order_jacobian_matrix_r_rtheta (
...
```C++
inline KOKKOS_INLINE_FUNCTION IdxRangeRTheta DiscretePoloidalCSSplineMapping::idx_range_singular_point () const
```
...
```C++
inline KOKKOS_FUNCTION double DiscretePoloidalCSSplineMapping::jacobian (
    Coord< curvilinear_tag_r , curvilinear_tag_theta > const & coord
) const
```
...
inline KOKKOS_INLINE_FUNCTION double DiscretePoloidalCSSplineMapping::jacobian_component (
    Coord< R , Theta > coord
) const
```
...
```C++
inline KOKKOS_FUNCTION DTensor < VectorIndexSet< X , Y >, VectorIndexSet< R_cov , Theta_cov > > DiscretePoloidalCSSplineMapping::jacobian_matrix (
...
```C++
inline KOKKOS_INLINE_FUNCTION Coord< X , Y > DiscretePoloidalCSSplineMapping::o_point () const
```
...
```C++
inline KOKKOS_FUNCTION Coord< X , Y > DiscretePoloidalCSSplineMapping::operator() (
    Coord< curvilinear_tag_r , curvilinear_tag_theta > const & coord
) const
```
...
inline void DiscretePoloidalCSSplineMapping::operator() (
    ExecSpace exec_space,
...
------------------------------
The documentation for this class was generated from the following file `/home/runner/work/gyselalibxx/gyselalibxx/code_branch/src/coord_transformations/discrete_poloidal_cs_spline_mapping.hpp`
