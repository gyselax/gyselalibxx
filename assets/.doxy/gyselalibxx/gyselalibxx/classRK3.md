...
| Type | Name |
| ---: | :--- |
|  KOKKOS\_DEFAULTED\_FUNCTION | [**RK3**](#function-rk3) () = default<br>_Create a_ [_**RK3**_](classRK3.md) _object to operate on scalars._ |
...
A class which provides an implementation of a third-order Runge-Kutta method in order to evolve values over time. This specialisation handles elementwise operations and can be called from GPU.
...
```C++
using RK3< ValType, DerivType, ExecSpace >::DerivFieldMem =  DerivType;
```
...
```C++
using RK3< ValType, DerivType, ExecSpace >::ValFieldMem =  ValType;
```
...
```C++
using RK3< ValType, DerivType, ExecSpace >::exec_space =  ExecSpace;
```
...
_Create a_ [_**RK3**_](classRK3.md) _object to operate on scalars._
```C++
explicit KOKKOS_DEFAULTED_FUNCTION RK3::RK3 () = default
```
...
inline KOKKOS_FUNCTION void RK3::update (
    ValType & y,
    double dt,
