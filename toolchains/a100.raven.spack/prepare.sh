...
if [ $# -ne 1 ]; then
    echo usage: ${0} a100_eaven_slurm_account
    exit 1
fi
slurm_account=${1}
...
srun --account=${slurm_account} --constraint=a100 --ntasks=1 --gres=gpu:${GPUS} --cpus-per-task=${CPUS_PER_TASK} --time=02:00:00 --qos=qos_gpu_a100-dev spack --env gyselalibxx-spack-environment concretize --force --jobs ${CPUS_PER_TASK}
...
GPUS=3
CPUS_PER_TASK=$((${GPUS} * ${CPUS_PER_GPU}))
srun --account=${slurm_account} --constraint=a100 --ntasks=1 --gres=gpu:${GPUS} --cpus-per-task=${CPUS_PER_TASK} --time=02:00:00 --qos=qos_gpu_a100-dev spack --env gyselalibxx-spack-environment install --concurrent-packages ${GPUS} --jobs ${CPUS_PER_GPU}
