...
public:
    PolarPoissonLikeCoefficients(
            SplineEvaluator_host evaluator_r,
    ...
    double alpha(const double& r, const double& theta) const final
    {
        return m_evaluator_r(CoordRTheta(r, theta), m_coeff_alpha);
    }
    double beta(const double& r, const double& theta) const final
    {
        return m_evaluator_r(CoordRTheta(r, theta), m_coeff_beta);
    }
