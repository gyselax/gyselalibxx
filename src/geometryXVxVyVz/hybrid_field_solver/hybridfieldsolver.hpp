 ...
 *
 * An operator which solves the field equation about magnetic field, pressure, and mean velocity
 * using a fast Fourier transform on a periodic index range.
 * This operator only works for equidistant points.
 */
class HybridFieldSolver : public IHybridFieldSolver
{
    using HybridSolver = IHybridSolver1d<
            IdxRangeX,
            IdxRangeX,
            typename Kokkos::DefaultExecutionSpace::memory_space,
            Kokkos::layout_right>;
    HybridSolver const& m_solve_hybrid;
    IMomentsCalculator const& m_compute_moments;

public:
    /**
     * Construct the nonlinear solver operator.
     *
     * @param solve_hybrid The operator which solves the nonlinear equation.
     * @param compute_moments The operator which calculates the charge density, and 1st order velocity moments.
     */
    HybridFieldSolver(HybridSolver const& solve_hybrid, IMomentsCalculator const& compute_moments);

    ~HybridFieldSolver() override = default;

    /**
     * The operator which solves the nonlinear equation using the method described by the class. 
     *
