...
extern "C" int dgbtrf_(const int *m, const int *n, const int *kl, const int *ku,
        double *a_b, const int *lda_b, int *ipiv, int *info);
extern "C" int dgbtrs_(const char *trans, const int *n, const int *kl, const int *ku,
        const int *nrhs, double *a_b, const int *lda_b, int *ipiv, double *b,
        const int *ldb, int *info);

Matrix_Banded::Matrix_Banded(int n, int kl, int ku)
    : Matrix(n), kl(kl), ku(ku), c(2*kl + ku + 1)
{
    assert( n  >  0 );
    assert( kl >= 0 );
    assert( ku >= 0 );
