...
Matrix_Corner_Block::Matrix_Corner_Block(int n,
    int k,
...
double
Matrix_Corner_Block::get_element(int i, int j) const
{
    assert(i >= 0);
    assert(i < n);
    assert(j >= 0);
...
void Matrix_Corner_Block::solve_lambda_section(mdspan_1d& u,
...
void Matrix_Corner_Block::solve_lambda_section(mdspan_2d& u,
...
void Matrix_Corner_Block::solve_lambda_section_transpose(mdspan_1d& u,
    mdspan_1d const& v) const
{
    for (int i(0); i < k; ++i) {
        // Upper diagonals in (*lambda)
