...
void Matrix_Center_Block::swap_array_to_corner(mdspan_2d& bx) const
{
    ...
    memcpy(swap_array.get(), bx.data() + top_block_size * ncols, q_block->get_size() * ncols*sizeof(double));
...
void Matrix_Center_Block::swap_array_to_center(mdspan_2d& bx) const
{
    ...
    memcpy(bx.data() + top_block_size * ncols, swap_array.get(), q_block->get_size() * ncols*sizeof(double));
}
