...
void Matrix::factorize()
{
    ...
    if (info < 0) {
        ...
        std::cerr << " U is exactly singular, and division by zero will occur if "
                     "it is used to";
...
Matrix::make_new_banded(int n, int kl, int ku, bool pds)
{
    if (kl == ku && kl == 1 && pds) {
...
std::unique_ptr<Matrix>
Matrix::make_new_periodic_banded(int n, int kl, int ku, bool pds)
{
    ...
    return std::make_unique<Matrix_Periodic_Banded>(
        n, kl, ku, std::move(block_mat));
}

std::unique_ptr<Matrix>
Matrix::make_new_block_with_banded_region(int n,
    int kl,
    int ku,
    bool pds,
    int block1_size,
    int block2_size)
{
    ...
    if (block2_size == 0) {
        return std::make_unique<Matrix_Corner_Block>(
            n, block1_size, std::move(block_mat));
    } else {
        return std::make_unique<Matrix_Center_Block>(
            n, block1_size, block2_size, std::move(block_mat));
    }
}
...
operator<<(std::ostream& o, const Matrix& m)
{
    ...
    for (int i(0); i < n; ++i) {
        for (int j(0); j < n; ++j) {
            o << std::fixed << std::setprecision(3) << std::setw(10)
              << m.get_element(i, j);
        }
        o << std::endl;
    }
