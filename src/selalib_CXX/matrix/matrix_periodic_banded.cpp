...
Matrix_Periodic_Banded::Matrix_Periodic_Banded(int n,
    int kl,
    int ku,
    std::unique_ptr<Matrix> q)
    : Matrix_Corner_Block(n,
        max(kl, ku),
        std::move(q),
        max(kl, ku) * (max(kl, ku) + 1))
    , kl(kl)
    , ku(ku)
{
}
...
double
Matrix_Periodic_Banded::get_element(int i, int j) const
{
    assert(i >= 0);
    assert(i < n);
    assert(j >= 0);
...
void Matrix_Periodic_Banded::solve_lambda_section(mdspan_1d& u,
    mdspan_1d const& v) const
{
    for (int i(0); i < k; ++i) {
        // Upper diagonals in lambda
        for (int j(0); j <= i; ++j) {
...
void Matrix_Periodic_Banded::solve_lambda_section(mdspan_2d& u,
    mdspan_2d const& v) const
{
    for (int i(0); i < k; ++i) {
        // Upper diagonals in lambda
        for (int j(0); j <= i; ++j) {
...
void Matrix_Periodic_Banded::solve_lambda_section_transpose(mdspan_1d& u,
    mdspan_1d const& v) const
{
    for (int i(0); i < k; ++i) {
        // Upper diagonals in lambda
