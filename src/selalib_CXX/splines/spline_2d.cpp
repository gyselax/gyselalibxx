...
Spline_2D::Spline_2D(const BSplines& bspl1, const BSplines& bspl2)
    : bcoef_ptr(
        new double[(bspl1.degree + bspl1.ncells) * (bspl2.degree + bspl2.ncells)])
    , bcoef(bcoef_ptr.get(),
          bspl1.degree + bspl1.ncells,
          bspl2.degree + bspl2.ncells)
    , bspl1(bspl1)
    , bspl2(bspl2)
{
}

bool Spline_2D::belongs_to_space(const BSplines& bspline1,
    const BSplines& bspline2) const
{
...
double
Spline_2D::eval_intern(double x1,
    double x2,
    const T1& bspl1,
    const T2& bspl2,
    mdspan_1d& vals1,
...
double
Spline_2D::eval(const double x1, const double x2) const
{
...
double
Spline_2D::eval_deriv(const double x1, const double x2) const
{
    ...
    if (bspl1.uniform) {
        if (bspl2.uniform) {
            ...
            return eval_intern<BSplines_uniform,
        ...
        if (bspl2.uniform) {
            return eval_intern<BSplines_non_uniform,
                BSplines_uniform,
                deriv1,
                deriv2>(
                x1,
...
void Spline_2D::eval_array(mdspan_2d const& x1,
    mdspan_2d const& x2,
    mdspan_2d& y) const
{
...
template <class T1,
    ...
    class T2,
    typename std::enable_if<std::is_base_of<BSplines, T2>::value>::type* = nullptr>
void Spline_2D::eval_array_loop(mdspan_2d const& x1,
    mdspan_2d const& x2,
    mdspan_2d& y) const
{
    double values1[bspl1.degree + 1];
    ...
    for (int i(0); i < x1.extent(0); ++i) {
        for (int j(0); j < x1.extent(1); ++j) {
            y(i, j) = eval_intern<T1, T2, false, false>(
                x1(i, j), x2(i, j), l_bspl1, l_bspl2, vals1, vals2);
        }
    }
}

template <bool deriv1, bool deriv2>
void Spline_2D::eval_array_deriv(mdspan_2d const& x1,
    mdspan_2d const& x2,
    mdspan_2d& y) const
{
    assert(x1.extent(0) == y.extent(0));
