...
double
Spline_1D::eval_intern_no_bcs(double x, const T& bspl, mdspan_1d& vals) const
{
    int jmin;
...
template <
    ...
    typename std::enable_if<std::is_base_of<BSplines, T>::value>::type* = nullptr>
double
Spline_1D::eval_intern(double x, const T& bspl, mdspan_1d& vals) const
{
...
template <
    ...
    typename std::enable_if<std::is_base_of<BSplines, T>::value>::type* = nullptr>
double
Spline_1D::eval_deriv_intern(double x, const T& bspl, mdspan_1d& vals) const
{
    int jmin;
...
Spline_1D::eval(double x) const
{
    ...
    if (bspl.uniform) {
        if (bspl.periodic) {
            return eval_intern<BSplines_uniform, true>(
                x, static_cast<const BSplines_uniform&>(bspl), vals);
        } else {
            return eval_intern<BSplines_uniform, false>(
                x, static_cast<const BSplines_uniform&>(bspl), vals);
        }
    } else {
        if (bspl.periodic) {
            return eval_intern<BSplines_non_uniform, true>(
                x, static_cast<const BSplines_non_uniform&>(bspl), vals);
        } else {
            return eval_intern<BSplines_non_uniform, false>(
                x, static_cast<const BSplines_non_uniform&>(bspl), vals);
        }
    }
}
...
double
Spline_1D::eval_deriv(double x) const
{
    double values[bspl.degree + 1];
    mdspan_1d vals(values, bspl.degree + 1);

    if (bspl.uniform)
        return eval_deriv_intern<BSplines_uniform>(
            x, static_cast<const BSplines_uniform&>(bspl), vals);
    else
        return eval_deriv_intern<BSplines_non_uniform>(
            x, static_cast<const BSplines_non_uniform&>(bspl), vals);
}
