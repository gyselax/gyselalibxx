...
BSplines_uniform::BSplines_uniform(int degree,
    bool periodic,
    double xmin,
    double xmax,
    int ncells)
...
void BSplines_uniform::eval_basis(double x,
    mdspan_1d& values,
    int& jmin,
    int deg) const
{
...
void BSplines_uniform::eval_basis_and_n_derivs(double x,
    int n,
    mdspan_2d& derivs,
    int& jmin) const
{
    ...
    std::experimental::mdspan<double, std::experimental::dynamic_extent, 2> a(
        a_ptr, degree + 1);
    double offset;
...
void BSplines_uniform::get_icell_and_offset(double x,
    int& icell,
    double& offset) const
{
    assert(x >= xmin);
