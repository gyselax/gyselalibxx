...
inline void
Spline_interpolator_1D::constructor_sanity_checks()
{
...
const mdspan_1d&
Spline_interpolator_1D::get_interp_points() const
{
    return interp_pts;
}
...
void Spline_interpolator_1D::compute_interpolation_points_non_uniform()
{
    ...
    if (xmin_bc == BoundaryCondition::sll_p_periodic) {
        ...
        for (int j(0); i < n_temp_knots; ++i, ++j) {
            ...
            if (xmax_bc == BoundaryCondition::sll_p_hermite)
                temp_knots[i] = 2.0 * bspl_nu.get_knot(bspl_nu.ncells) - bspl_nu.get_knot(bspl_nu.ncells - 1 - j);
        }
    }
...
void Spline_interpolator_1D::compute_block_sizes_uniform(int& lower_block_size,
...
void Spline_interpolator_1D::allocate_matrix(int lower_block_size,
    int upper_block_size)
{
    ...
    if (xmin_bc == BoundaryCondition::sll_p_periodic) {
        matrix = Matrix::make_new_periodic_banded(
            bspl.nbasis, upper_band_width, upper_band_width, bspl.uniform);
    } else {
        matrix = Matrix::make_new_block_with_banded_region(bspl.nbasis,
            upper_band_width,
            upper_band_width,
            bspl.uniform,
            upper_block_size,
