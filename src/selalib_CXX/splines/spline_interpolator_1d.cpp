...
std::array<BoundaryCondition, 3> Spline_interpolator_1D::allowed_bcs = {
    BoundaryCondition::sll_p_periodic, BoundaryCondition::sll_p_hermite, BoundaryCondition::sll_p_greville};

Spline_interpolator_1D::Spline_interpolator_1D(BSplines* bspl,
...
void Spline_interpolator_1D::compute_interpolant_degree1(Spline_1D& spline,
...
void Spline_interpolator_1D::compute_interpolant(Spline_1D& spline,
        const mdspan_1d& vals, const mdspan_1d* derivs_xmin,
        const mdspan_1d* derivs_xmax) const
{
    assert( vals.extent(0) == bspl->nbasis - nbc_xmin - nbc_xmax );
    assert( spline.belongs_to_space(bspl) );
    // TODO: LOG Errors
    if (bspl->degree == 1) return compute_interpolant_degree1(spline, vals);

    assert( (xmin_bc == BoundaryCondition::sll_p_hermite) != (derivs_xmin == nullptr || derivs_xmin->extent(0) == 0) );
    assert( (xmax_bc == BoundaryCondition::sll_p_hermite) != (derivs_xmax == nullptr || derivs_xmax->extent(0) == 0) );
    ...
    mdspan_1d bcoef_section (spline.bcoef_ptr+offset, bspl->nbasis);
...
void Spline_interpolator_1D::compute_interpolation_points_uniform( )
{
    ...
    interp_pts_ptr = new double[n_interp_pts];
    interp_pts = mdspan_1d(interp_pts_ptr,n_interp_pts);
    ...
    else
    {
        ...
        {
            if (xmin_bc == BoundaryCondition::sll_p_greville) iknots[i] = 0;
            if (xmin_bc == BoundaryCondition::sll_p_hermite)  iknots[i] = -n_to_fill_min + i;
        }
        ...
        {
            if (xmax_bc == BoundaryCondition::sll_p_greville) iknots[i] = bspl->ncells;
            if (xmax_bc == BoundaryCondition::sll_p_hermite)  iknots[i] = bspl->ncells + j;
        }
...
void Spline_interpolator_1D::compute_interpolation_points_non_uniform ( )
{
    ...
    else
    {
        ...
        for (; i < n_start_pts ; ++i)
        {
            ...
            if (xmin_bc == BoundaryCondition::sll_p_greville) temp_knots[i] =       bspl_nu->get_knot(0);
            if (xmin_bc == BoundaryCondition::sll_p_hermite)  temp_knots[i] = 2.0 * bspl_nu->get_knot(0) \
                                    - bspl_nu->get_knot(n_start_pts - i);
        }
        ...
        {
            if (xmax_bc == BoundaryCondition::sll_p_greville) temp_knots[i] =       bspl_nu->get_knot(bspl_nu->ncells);
            if (xmax_bc == BoundaryCondition::sll_p_hermite)  temp_knots[i] = 2.0 * bspl_nu->get_knot(bspl_nu->ncells) \
...
void Spline_interpolator_1D::compute_block_sizes_uniform(int& lower_block_size, int& upper_block_size) const
{
    switch(xmin_bc)
    {
        case BoundaryCondition::sll_p_periodic: upper_block_size = (bspl->degree)/2; break;
        case BoundaryCondition::sll_p_hermite : upper_block_size = nbc_xmin;         break;
        case BoundaryCondition::sll_p_greville: upper_block_size = bspl->degree-1;   break;
        default: break; //TODO: throw error
    }
    switch(xmax_bc)
    {
        case BoundaryCondition::sll_p_periodic: lower_block_size = (bspl->degree)/2; break;
        case BoundaryCondition::sll_p_hermite : lower_block_size = nbc_xmax;         break;
        case BoundaryCondition::sll_p_greville: lower_block_size = bspl->degree-1;   break;
        default: break; //TODO: throw error
    }
}
...
void Spline_interpolator_1D::compute_block_sizes_non_uniform(int& lower_block_size, int& upper_block_size) const
{
    switch(xmin_bc)
    {
        case BoundaryCondition::sll_p_periodic: upper_block_size = (bspl->degree+1)/2; break;
        case BoundaryCondition::sll_p_hermite : upper_block_size = nbc_xmin+1;         break;
        case BoundaryCondition::sll_p_greville: upper_block_size = bspl->degree-1;     break;
        default: break; //TODO: throw error
    }
    switch(xmax_bc)
    {
        case BoundaryCondition::sll_p_periodic: lower_block_size = (bspl->degree+1)/2; break;
        case BoundaryCondition::sll_p_hermite : lower_block_size = nbc_xmax+1;         break;
        case BoundaryCondition::sll_p_greville: lower_block_size = bspl->degree-1;     break;
        default: break; //TODO: throw error
    }
}
...
void Spline_interpolator_1D::allocate_matrix(int lower_block_size, int upper_block_size)
{
    ...
    if (xmin_bc == BoundaryCondition::sll_p_periodic)
    {
        matrix = Matrix::make_new_periodic_banded(bspl->nbasis, upper_band_width, upper_band_width, bspl->uniform);
    }
    else
    {
        matrix = Matrix::make_new_block_with_banded_region(bspl->nbasis, upper_band_width, upper_band_width,
                bspl->uniform, upper_block_size, lower_block_size);
    }
...
int Spline_interpolator_1D::compute_num_cells(int degree,
        BoundaryCondition xmin_bc, BoundaryCondition xmax_bc,
        int nipts)
{
    assert( degree > 0 );
    ...
    if ((xmin_bc == BoundaryCondition::sll_p_periodic) != (xmax_bc == BoundaryCondition::sll_p_periodic))
    {
    ...
    if (xmin_bc == BoundaryCondition::sll_p_periodic)
    {
        ...
        if (xmin_bc == BoundaryCondition::sll_p_hermite) nbc_xmin = degree/2;
        else nbc_xmin = 0;

        if (xmax_bc == BoundaryCondition::sll_p_hermite) nbc_xmax = degree/2;
        else nbc_xmax = 0;
...
{
    return new Spline_interpolator_1D(bspl, xmin_bc, xmax_bc);
}
...
{
    return Spline_interpolator_1D::compute_num_cells(degree, xmin, xmax, nipts);
}

void compute_interpolant(const Spline_interpolator_1D* spl_interp, Spline_1D* spline,
        double* vals_ptr, int nvals, double* derivs_xmin_ptr,
        int nderivs_xmin, double* derivs_xmax_ptr, int nderivs_xmax)
{
    mdspan_1d vals(vals_ptr, nvals);
    mdspan_1d* derivs_xmin(nullptr);
    mdspan_1d* derivs_xmax(nullptr);

    if (derivs_xmin_ptr != nullptr)
    {
        derivs_xmin = new mdspan_1d(derivs_xmin_ptr, nderivs_xmin);
    }
    if (derivs_xmax_ptr != nullptr)
    {
        derivs_xmax = new mdspan_1d(derivs_xmax_ptr, nderivs_xmax);
    }

    spl_interp->compute_interpolant(*spline, vals, derivs_xmin, derivs_xmax);

    if (derivs_xmin != nullptr) delete derivs_xmin;
    if (derivs_xmax != nullptr) delete derivs_xmax;
}

void get_interp_points(Spline_interpolator_1D* spl_interp, double* interp_points, int npts)
{
    mdspan_1d pts (spl_interp->get_interp_points());
    assert( npts == pts.extent(0) );
    for (int i(0); i < npts; ++i)
    {
        interp_points[i] = pts(i);
    }
}
