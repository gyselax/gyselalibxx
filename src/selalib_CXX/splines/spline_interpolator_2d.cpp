 ...
 ******************************************************************************/
std::array<const mdspan_1d, 2>
Spline_interpolator_2D::get_interp_points() const
{
 ...
 ******************************************************************************/
std::array<int, 2>
Spline_interpolator_2D::compute_num_cells(std::array<int, 2> degree,
    std::array<BoundaryCondition, 2> xmin,
    std::array<BoundaryCondition, 2> xmax,
    std::array<int, 2> nipts)
{
    int n0(Spline_interpolator_1D::compute_num_cells(
        degree[0], xmin[0], xmax[0], nipts[0]));
    int n1(Spline_interpolator_1D::compute_num_cells(
        degree[1], xmin[1], xmax[1], nipts[1]));
    return { n0, n1 };
}
    ...
    Boundary_data_2d boundary_data) const
{
    ...
    if (nbc_xmax[0] > 0 and nbc_xmax[1] > 0) {
        ...
        for (int i(0); i < nbc_xmax[0]; ++i) {
            for (int j(0); j < nbc_xmax[1]; ++j) {
                spline.bcoef(i + bspl[0]->nbasis - nbc_xmax[0],
                    j + bspl[1]->nbasis - nbc_xmax[1])
    ...
    mdspan_2d const& vals) const
{
    ...
    std::array<Spline_1D, 2> spline_1d(
        { Spline_1D(*bspl[0]), Spline_1D(*bspl[1]) });
    double t_storage_ptr[spline.bcoef.extent(0) * spline.bcoef.extent(1)];
    mdspan_2d t_storage(
        t_storage_ptr, spline.bcoef.extent(1), spline.bcoef.extent(0));
    ...
    // and interpolate f along x2 direction. Store coefficients in bcoef
    {
        ...
        for (; i < bspl[0]->nbasis; ++i) {
            ...
            mdspan_1d derivs_xmax(&spline.bcoef(i, bspl[1]->nbasis - nbc_xmax[1]),
                nbc_xmax[1]);
            interp_1d[1].compute_interpolant(
    ...
    // and interpolate f along x1 direction. Store coefficients in bwork
    {
        ...
        for (; j < bspl[1]->nbasis; ++j) {
            ...
            mdspan_1d derivs_xmax(&t_storage(j, bspl[0]->nbasis - nbc_xmax[0]),
                nbc_xmax[0]);
            interp_1d[0].compute_interpolant(
                spline_1d[0], values, &derivs_xmin, &derivs_xmax);
