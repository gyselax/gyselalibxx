...
BSplines_non_uniform::BSplines_non_uniform(int degree,
    bool periodic,
    ...
    : BSplines(degree,
        ...
        periodic ? (breaks.size() - 1)
                 : (breaks.size() - 1 + degree), // nbasis
        breaks[0], // xmin
        breaks.back(), // xmax
        false) // radial
{
...
void BSplines_non_uniform::eval_basis_and_n_derivs(double x,
    int n,
    mdspan_2d& derivs,
    int& jmin) const
{
