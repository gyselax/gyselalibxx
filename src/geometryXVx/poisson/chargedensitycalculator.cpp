...
DSpanX ChargeDensityCalculator::operator()(DSpanX const rho, DViewSpXVx const allfdistribu) const
{
    ...
    host_t<ViewSp<double>> const charges = ddc::host_discrete_space<IDimSp>().charges();
    host_t<ViewSp<double>> const kinetic_charges = charges[allfdistribu.domain<IDimSp>()];

    auto charges_alloc
            = create_mirror_view_and_copy(Kokkos::DefaultExecutionSpace(), kinetic_charges);

    Kokkos::View<const double*, Kokkos::LayoutRight> const charges_view
                ...
                Kokkos::parallel_reduce(
                        ...
                        [&](int const& ivx, double& sum) {
                            ...
                            for (std::size_t isp = 0; isp < nsp; isp++) {
                                sum += charges_view(isp) * coef_view(ivx)
