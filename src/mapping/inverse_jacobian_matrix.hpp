...
public:
    ...
    KOKKOS_INLINE_FUNCTION double inv_jacobian_component(PositionCoordinate const& coord) const
    {
        ...
        if constexpr (has_2d_inv_jacobian_v<Mapping, PositionCoordinate>) {
            ...
            if constexpr (std::is_same_v<IndexTag1, DimRes0> && std::is_same_v<IndexTag2, DimArg0_cov>) {
            ...
            else if constexpr (std::is_same_v<IndexTag1, DimRes0> && std::is_same_v<IndexTag2, DimArg1_cov>) {
            ...
            else if constexpr (std::is_same_v<IndexTag1, DimRes1> && std::is_same_v<IndexTag2, DimArg0_cov>) {
