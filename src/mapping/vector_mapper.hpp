...
public:
                    ...
                    LayoutStridedPolicy2> vector_field_input)
    {
        ...
        if constexpr (std::is_same_v<Coord<XIn, YIn>, typename Mapping::CoordArg>) {
            ...
            InverseJacobianMatrix<Mapping, typename Mapping::CoordJacobian> inv_mapping(m_mapping);
            using CoordIdxType = find_idx_t<typename Mapping::CoordJacobian, IdxRangeType>;
            ddc::parallel_for_each(
                    exec_space,
                    get_idx_range(vector_field_input),
                    KOKKOS_LAMBDA(IdxType idx) {
                        Tensor map_J = inv_mapping(ddc::coordinate(CoordIdxType(idx)));
