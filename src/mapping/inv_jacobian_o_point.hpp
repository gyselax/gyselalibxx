...
public:
     ...
     */
    KOKKOS_INLINE_FUNCTION DTensor<VectorIndexSet<R, Theta>, VectorIndexSet<X_cov, Y_cov>>
    operator()() const
    {
        DTensor<VectorIndexSet<R, Theta>, VectorIndexSet<X_cov, Y_cov>> J;
        ddcHelper::get<R, X_cov>(J) = 1.;
        ddcHelper::get<R, Y_cov>(J) = 0.;
        ddcHelper::get<Theta, X_cov>(J) = 0.;
        ddcHelper::get<Theta, Y_cov>(J) = 1.;
        return J;
    }
};
...
public:
    ...
    operator()() const
    {
        ...
        DTensor<VectorIndexSet<R, Theta>, VectorIndexSet<X_cov, Y_cov>> J;
        ddcHelper::get<R, X_cov>(J) = -sqrt_eps_2;
        ddcHelper::get<R, Y_cov>(J) = 0.;
        ddcHelper::get<Theta, X_cov>(J) = 0.;
        ddcHelper::get<Theta, Y_cov>(J) = (2 - sqrt_eps_2) / e / xi;
        return J;
    }
};
    ...
    operator()() const
    {
        DiscreteToCartesian<X, Y, SplineEvaluator, R, Theta, MemorySpace> const& discrete_mapping
                = m_mapping.template get<
                        DiscreteToCartesian<X, Y, SplineEvaluator, R, Theta, MemorySpace>>();
        DTensor<VectorIndexSet<R, Theta>, VectorIndexSet<X_cov, Y_cov>> J(0.0);
        ...
        for (IdxTheta ip : IdxRangeTheta(idx_range_singular_point)) {
            ...
            DTensor<VectorIndexSet<R, Theta>, VectorIndexSet<X_cov, Y_cov>> J_circ_r_rtheta;
            ddcHelper::get<R, X_cov>(J_circ_r_rtheta) = Kokkos::cos(th);
            ddcHelper::get<R, Y_cov>(J_circ_r_rtheta) = Kokkos::sin(th);
            ddcHelper::get<Theta, X_cov>(J_circ_r_rtheta) = -Kokkos::sin(th);
            ddcHelper::get<Theta, Y_cov>(J_circ_r_rtheta) = Kokkos::cos(th);

            Tensor J_theta = inverse(mat_mul(J_first_order, J_circ_r_rtheta));
