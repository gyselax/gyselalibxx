...
/**
 * @brief A class which computes the velocity advection in vx of a 2D shifted rotation. Working for every cartesian geometry.
 */

 template <class Geometry, class GridV>
 class BslAdvectionVelocityRot2DVx : public IAdvectionVelocityRot2D<Geometry, GridV>
 {
 ...
 public:
      ...
      */
     explicit BslAdvectionVelocityRot2DVx(PreallocatableInterpolatorType const& interpolator_v)
         : m_interpolator_v(interpolator_v)
     {
     }
 
     ~BslAdvectionVelocityRot2DVx() override = default;
             ...
             double const dt) const override
     {
         ...
         Kokkos::Profiling::pushRegion("BslAdvectionVelocityRot2DVx");
         ...
         ddc::for_each(idx_range_sp, [&](IdxSp const isp) {
             ...
             ddc::parallel_for_each(
                     ...
                     KOKKOS_LAMBDA(IdxBatch const ib) {
                         ...
                         //double const dvx
                         //        = -1.0 * charge_proxy * sqrt_me_on_mspecies * (ddc::coordinate(ivy) - mean_velocity_y(ix)) * std::tan(0.5*dt*magnetic_field_z(ix));
                        //double const dvx
                        //         = charge_proxy / mass(isp)  * (ddc::coordinate(ivy) + mean_velocity_y(ix)) * std::tan(-0.5*dt*magnetic_field_z(ix));
                        double const dvx
                                 = (ddc::coordinate(ivy) + mean_velocity_y(ix)) * std::tan(-0.5*dt*charge_proxy/mass(isp)*magnetic_field_z(ix));
                         //std::cout << "check rot 2d vx is: " << std::setprecision(16) << charge_proxy / mass(isp) << std::endl;
