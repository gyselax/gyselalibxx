...
def Compute_POPEop_forPoisson(
    ...
    dx = xgrid[1] - xgrid[0]         # ATTENTION only valid for uniform mesh
    ...
    rho = (ni - ne)
    #A REMETTRE: rho = mask_rho[None,:]*(ni - ne)
    ...
    for it_op in range(nt_op):
        ...
        [d2xPhi_POPE[it_op,:],x] = MATHut.iFourier1D(FFT2Phi_POPE,k)
...
def Get_POPEop_forPoisson(
    ...
    if os.path.exists(POPE_h5filename):
        dic_POPEop_Poisson_exist = H5ut.group_exist_in_hdf5( POPE_h5filename, h5group_Poisson )
        if dic_POPEop_Poisson_exist:
            print('==> Loading of POPE operators for Poisson in '+POPE_h5filename)
            dic_POPEop_Poisson = H5ut.load_dict_from_hdf5( POPE_h5filename, h5group_Poisson )
            return dic_POPEop_Poisson
...
def Compute_POPEop_forVlasovLHS(
    ...
    for it_op in range(nt_op):
        ...
        [FFTPhi,k] = MATHut.Fourier1D(Phi[it,:],xgrid)
        FFTPhi = 1.j*k*FFTPhi
        [dxPhi[it_op,:],x] = MATHut.iFourier1D(FFTPhi,k)
    ...
    for it_op in range(nt_op):
        ...
        for iv in range(nv):
            ...
            [v_dxfelec[it_op,:,iv],x] = MATHut.iFourier1D(FFTxfelec,kfelec)
    ...
    if fion_exist:
        ...
        for it_op in range(nt_op):
            ...
            for iv in range(nv):
                ...
                [v_dxfion[it_op,:,iv],x] = MATHut.iFourier1D(FFTxfion,kfion)
...
def Get_POPEop_forVlasovLHS(
    ...
    if os.path.exists(POPE_h5filename):
        dic_POPEop_Vlasov_exist = H5ut.group_exist_in_hdf5( POPE_h5filename, h5group_Vlasov )
        if dic_POPEop_Vlasov_exist:
            print('==> Loading of POPE operators for Vlasov in '+POPE_h5filename)
            dic_POPEop_Vlasov = H5ut.load_dict_from_hdf5( POPE_h5filename, h5group_Vlasov )
